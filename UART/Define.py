import time
configuration=['0010','0101',8,2,3]

def configure(start_squence, stop_sequence, speed_data, length_data_index, packet_rate):
    configuration[0]=start_squence
    configuration[1]=stop_sequence
    configuration[2]=speed_data
    configuration[3]=length_data_index*8
    configuration[4]=packet_rate
...
def parity(message):
    parity=0
    for i in message:
        parity=parity^(ord(i)-ord('0'))
    return str(parity)

def parityCheck(message, value):
...
def decryptpacket(buffer):
    ...
    if parityCheck(data, paritybit) and start==configuration[0] and stop==configuration[1]:
